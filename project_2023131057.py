...
def evaluate_hand(hand, community):
    ...
    if len(unique) >= 5:
        for i in range(len(unique) - 4):
            if all(unique[i + j] - unique[i + j - 1] == 1 for j in range(1, 5)):
                is_straight = True
...
class SniperHoldem:
        ...
        for sniper_name, guess_combo, guess_rank in guesses:
            for player in self.players:
                ...
                if combo == guess_combo and rank == guess_rank:
                    print(f"{sniper_name}의 저격 성공: {player.name}의 '{guess_rank} {combo}' 저격됨")
                    banned.append((combo, rank))
        if banned:
            self.banned_combo = banned[-1]
        else:
    ...
    def resolve_round(self):
        ...
        self.just_eliminated = [p.name for p in self.players if not p.eliminated and p.chips == 0]
        if not self.just_eliminated:
            print(f"{winner.name}가 팟을 획득했습니다. 남은 칩: {winner.chips}")
    # 탈락자 확인
    def check_elimination(self):
        for p in self.players:
            if p.chips <= 0 and not p.eliminated:
                p.eliminated = True
                print(f"{p.name} 탈락!")
            elif p.chips >= 75 and not p.eliminated:
                surplus = p.chips - 75
                p.chips = 75
                p.eliminated = True
        ...
        if len(active_players) == 1:
            ...
            last_player.eliminated = True
            print(f"{last_player.name}는 혼자 남아 자동 탈락하였습니다!")
    ...
    def play_game(self):
        ...
        while True:
            ...
            if len(self.just_eliminated) > 0:
                print("\n[최종 탈락자 정보]")
                for name in self.just_eliminated:
                    print(f"{name} 최종 탈락")
                print("\n게임 종료! 최종 탈락자가 발생했습니다.")
                break
            round_count += 1
